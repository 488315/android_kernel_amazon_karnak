...
static const struct mtk_composite top_muxes[] __initconst = {
	/* CLK_CFG_0 */
	MUX_GATE(CLK_TOP_AXI_SEL, "axi_sel", axi_parents, 0x0040, 0, 1, 7),
	...
	MUX_GATE(CLK_TOP_MM_SEL, "mm_sel", mm_parents, 0x0040, 24, 2, 31),
	/* CLK_CFG_1 */
	MUX_GATE(CLK_TOP_PWM_SEL, "pwm_sel", pwm_parents, 0x0050, 0, 2, 7),
	MUX_GATE(CLK_TOP_VDEC_SEL, "vdec_sel", vdec_parents, 0x0050, 8, 3, 15),
	MUX_GATE(CLK_TOP_MFG_SEL, "mfg_sel", mfg_parents, 0x0050, 24, 2, 31),
	/* CLK_CFG_2 */
	MUX_GATE(CLK_TOP_CAMTG_SEL, "camtg_sel", camtg_parents, 0x0060, 0, 2, 7),
	MUX_GATE(CLK_TOP_UART_SEL, "uart_sel", uart_parents, 0x0060, 8, 1, 15),
	MUX_GATE(CLK_TOP_SPI_SEL, "spi_sel", spi_parents, 0x0060, 16, 1, 23),
	/* CLK_CFG_3 */
	MUX_GATE(CLK_TOP_MSDC30_0_SEL, "msdc30_0_sel", msdc30_0_parents, 0x0070, 0, 3, 7),
	MUX_GATE(CLK_TOP_MSDC30_1_SEL, "msdc30_1_sel", msdc30_1_parents, 0x0070, 8, 3, 15),
	MUX_GATE(CLK_TOP_MSDC30_2_SEL, "msdc30_2_sel", msdc30_2_parents, 0x0070, 16, 3, 23),
	/* CLK_CFG_4 */
	MUX_GATE(CLK_TOP_MSDC50_3_HSEL, "msdc50_3_hsel", msdc50_3_h_parents, 0x0080, 0, 2, 7),
	MUX_GATE(CLK_TOP_MSDC50_3_SEL, "msdc50_3_sel", msdc50_3_parents, 0x0080, 8, 4, 15),
	MUX_GATE(CLK_TOP_AUDIO_SEL, "audio_sel", audio_parents, 0x0080, 16, 2, 23),
	MUX_GATE(CLK_TOP_AUD_INTBUS_SEL, "aud_intbus_sel", aud_intbus_parents, 0x0080, 24, 2, 31),
	/* CLK_CFG_5 */
	MUX_GATE(CLK_TOP_PMICSPI_SEL, "pmicspi_sel", pmicspi_parents, 0x0090, 0, 1, 7),
	MUX_GATE(CLK_TOP_SCP_SEL, "scp_sel", scp_parents, 0x0090, 8, 2, 15),
	MUX_GATE(CLK_TOP_ATB_SEL, "atb_sel", atb_parents, 0x0090, 16, 1, 23),
	MUX_GATE(CLK_TOP_MJC_SEL, "mjc_sel", mjc_parents, 0x0090, 24, 2, 31),
	/* CLK_CFG_6 */
	MUX_GATE(CLK_TOP_DPI0_SEL, "dpi0_sel", dpi0_parents, 0x00a0, 0, 3, 7),
	MUX_GATE(CLK_TOP_SCAM_SEL, "scam_sel", scam_parents, 0x00a0, 8, 1, 15),
	MUX_GATE(CLK_TOP_AUD_1_SEL, "aud_1_sel", aud_1_parents, 0x00a0, 16, 1, 23),
	MUX_GATE(CLK_TOP_AUD_2_SEL, "aud_2_sel", aud_2_parents, 0x00a0, 24, 1, 31),
	/* CLK_CFG_7 */
	MUX_GATE(CLK_TOP_DPI1_SEL, "dpi1_sel", dpi1_parents, 0x00b0, 0, 3, 7),
	MUX_GATE(CLK_TOP_UFOENC_SEL, "ufoenc_sel", ufoenc_parents, 0x00b0, 8, 3, 15),
	MUX_GATE(CLK_TOP_UFODEC_SEL, "ufodec_sel", ufodec_parents, 0x00b0, 16, 3, 23),
	MUX_GATE(CLK_TOP_ETH_SEL, "eth_sel", eth_parents, 0x00b0, 24, 4, 31),
	/* CLK_CFG_8 */
	MUX_GATE(CLK_TOP_ONFI_SEL, "onfi_sel", onfi_parents, 0x00c0, 0, 3, 7),
	MUX_GATE(CLK_TOP_SNFI_SEL, "snfi_sel", snfi_parents, 0x00c0, 8, 2, 15),
	MUX_GATE(CLK_TOP_HDMI_SEL, "hdmi_sel", hdmi_parents, 0x00c0, 16, 2, 23),
	MUX_GATE(CLK_TOP_RTC_SEL, "rtc_sel", rtc_parents, 0x00c0, 24, 2, 31),
};
...
static void mtk_clk_enable_critical(void)
{
	...
	clk_prepare_enable(clk_hw_get_clk(top_data->hws[CLK_TOP_AXI_SEL], "axi_sel"));
	clk_prepare_enable(clk_hw_get_clk(top_data->hws[CLK_TOP_MEM_SEL], "mem_sel"));
	clk_prepare_enable(clk_hw_get_clk(top_data->hws[CLK_TOP_DDRPHYCFG_SEL], "ddrphycfg_sel"));
	clk_prepare_enable(clk_hw_get_clk(top_data->hws[CLK_TOP_RTC_SEL], "rtc_sel"));
}

static void __init mtk_topckgen_init(struct platform_device *pdev)
{
...
static void __init mtk_infrasys_init(struct platform_device *pdev)
{
	struct clk_hw_onecell_data *clk_data;
	...
	mtk_clk_register_composites(&pdev->dev, infra_muxes, 
				ARRAY_SIZE(infra_muxes), base,
				&mt8163_clk_lock, clk_data);
...
static void __init mtk_apmixedsys_init(struct platform_device *pdev)
{
    struct clk_hw_onecell_data *clk_data;
    void __iomem *base;
    int r, i;
    ...
    mtk_clk_register_plls(pdev->dev.of_node, plls, ARRAY_SIZE(plls), clk_data);
    ...
    r = of_clk_add_provider(pdev->dev.of_node, of_clk_src_onecell_get, clk_data);
    if (r)
        pr_err("%s(): could not register clock provider: %d\n",
               __func__, r);
...
static int mtk_mfgsys_init(struct device_node *node)
{
	struct clk_hw_onecell_data *clk_data;
	int r;
...
static void __init mtk_audiosys_init(struct platform_device *pdev)
{
    struct clk_hw_onecell_data *clk_data;
    int r;
    ...
    mtk_clk_register_gates(&pdev->dev, pdev->dev.of_node, audio_clks,
...
static const struct of_device_id of_match_clk_mt8163[] = {
    {
        ...
        .compatible = "mediatek,mt8163-audiosys", // Add the new entry
        .data = mtk_audiosys_init,                // Specify the corresponding init function
    }, {
        /* sentinel */
    }
};
MODULE_DEVICE_TABLE(of, of_match_clk_mt8163);

static int clk_mt8163_probe(struct platform_device *pdev)
{
...
static struct platform_driver clk_mt8163_drv = {
	.probe = clk_mt8163_probe,
	.driver = {
		.name = "clk-mt8163",
		.of_match_table = of_match_clk_mt8163,
	},
};
...
static int __init clk_mt8163_init(void)
{
	timer_ready = true;
	mtk_clk_enable_critical();
	return platform_driver_register(&clk_mt8163_drv);
}
