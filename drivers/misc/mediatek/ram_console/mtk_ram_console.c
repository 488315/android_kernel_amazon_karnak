		...
		void *v)
{
	...
	while ((size = psi->read(&record)) > 0) {
		// Access record.id, record.type, record.count, record.buf, etc.
...
static int ram_console_proc_open(struct inode *inode, struct file *file)
{
    return ram_console_file_open(inode, file);
}
// Define the proc file operations structure with the proc_open function
static const struct proc_ops ram_console_proc_ops = {
    .proc_open = ram_console_proc_open,
    .proc_read = seq_read,
...
static int __init ram_console_late_init(void)
{
	...
	entry = proc_create("last_kmsg", 0444, NULL, &ram_console_proc_ops);
	if (!entry) {
...
int aee_rr_reboot_reason_show(struct seq_file *m, void *v)
{
	...
	if (ram_console_check_header(ram_console_old)) {
		...
		for (i = 0; i < array_sz(aee_rr_last_xxx); i++)
			aee_rr_last_xxx[i] (m);
		return 0;
	}
	for (i = 0; i < array_sz(aee_rr_show); i++)
		aee_rr_show[i] (m);

	for (cpu = 0; cpu < num_possible_cpus(); cpu++) {
		seq_printf(m, "CPU %d\n", cpu);
		for (i = 0; i < array_sz(aee_rr_show_cpu); i++)
